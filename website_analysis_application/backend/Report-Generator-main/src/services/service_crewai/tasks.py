...
def create_tasks(agents: Dict[str, Agent], url: str) -> List[Task]:
    ...
    image_analysis_task = Task(
        description=(
            f"Analyze the visual design of the image assets on the {url} webpage. Identify design inconsistencies and accessibility issues."
        ),
        expected_output=(
    ...
    ui_ux_analysis_task = Task(
        description=(
            f"Evaluate the design, usability, accessibility, and responsiveness of the {url} webpage. Focus on WCAG standards, media queries, and interactive components."
        ),
        expected_output=(
    ...
    seo_analysis_task = Task(
        description=(
            f"Perform a technical SEO audit of the {url} webpage. Focus on Core Web Vitals, structured data, indexing, and metadata optimization."
        ),
        expected_output=(
    ...
    seo_report_task = Task(
        description=(
            "Compile a concise SEO report summarizing findings for {url}. Include key recommendations for improvements."
        ),
        expected_output=(
            "Structured report: 1) Summary of issues, 2) Key Technical Explanations, 3) Prioritized Action Plan."
        ),
